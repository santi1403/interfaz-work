  ...
  req.on('end', async ()=>{
    try{
      ...
      if(req.method==='PUT'){
        ...
        const estadoUpd=d.estado||'activo'; await pool.query('UPDATE clientes SET num_cliente=?,tipo_id=?,identificacion=?,nombre=?,apellido=?,direccion=?,telefono=?,pais=?,fecha_desde=?,fecha_nacimiento=?,estado=? WHERE id=?',[d.numCliente,d.tipoCliente,d.identificacion,d.nombre,d.apellido,d.direccion,d.telefono,d.pais,d.fechaDesde,d.fechaNacimiento,estadoUpd,id]);
